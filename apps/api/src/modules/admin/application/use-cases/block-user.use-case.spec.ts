...
const blockedSummary: AdminUserSummary = {
  id: 'target-1',
...
function createUseCase(options?: {
  ...
  const adminFindById = jest
    .fn()
    .mockResolvedValue(
      options?.target === undefined ? blockedSummary : options.target,
    );
...
describe('BlockUserUseCase', () => {
  ...
  it('throws VALIDATION_ERROR when admin blocks self', async () => {
    const { useCase } = createUseCase();

    await expect(
      useCase.execute({ currentUser: adminUser, userId: 'admin-1' }),
