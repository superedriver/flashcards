...
export type AddStudyLanguageUseCaseInput = {
  currentUserId: string;
  languageCode: string;
};

export type AddStudyLanguageUseCaseResult = UserStudyLanguageListItem;

@Injectable()
export class AddStudyLanguageUseCase {
  constructor(
  ...
  async execute(
    input: AddStudyLanguageUseCaseInput,
  ): Promise<AddStudyLanguageUseCaseResult> {
    await this.ensureActiveUser(input.currentUserId);
    await this.ensureLanguageExists(input.languageCode);

    const studyLanguage = await this.userStudyLanguageRepository.upsert(
      input.currentUserId,
      input.languageCode,
    );
