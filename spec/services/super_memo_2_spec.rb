...
describe "SuperMemo2 service" do
  ...
  describe "#check_translation" do
    describe "correct case" do
      ...
      describe "perfect response" do
        ...
        it "the first attempt" do
          SuperMemo2.new(@card, @BEFORE_HESITATION).check_translation(@inputed_text)
        ...
        it "the second attempt" do
          2.times{ SuperMemo2.new(@card, @BEFORE_HESITATION).check_translation(@inputed_text) }
        ...
        it "the third attempt" do
          3.times{ SuperMemo2.new(@card, @BEFORE_HESITATION).check_translation(@inputed_text) }
        ...
        it "the fourth attempt" do
          4.times{ SuperMemo2.new(@card, @BEFORE_HESITATION).check_translation(@inputed_text) }
      ...
      describe "correct response after a hesitation" do
        ...
        it "the first attempt" do
          SuperMemo2.new(@card, @HESITATION).check_translation(@inputed_text)
          expect(@card[:current_step]).to eq(1)
        ...
        it "the second attempt" do
          2.times{ SuperMemo2.new(@card, @HESITATION).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(2)
          expect(@card[:e_factor]).to eq(2.5)
          expect(@card[:review_date]).to eq(6.day.from_now)
        end
        ...
        it "the third attempt" do
          3.times{ SuperMemo2.new(@card, @HESITATION).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(3)
          expect(@card[:e_factor]).to eq(2.5)
        ...
        it "the fourth attempt" do
          4.times{ SuperMemo2.new(@card, @HESITATION).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(4)
          expect(@card[:e_factor]).to eq(2.5)
      ...
      describe "correct response recalled with serious difficulty" do
        ...
        it "the first attempt" do
          SuperMemo2.new(@card, @DIFFICULTY).check_translation(@inputed_text)
          expect(@card[:current_step]).to eq(1)
        ...
        it "the second attempt" do
          2.times{ SuperMemo2.new(@card, @DIFFICULTY).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(2)
        ...
        it "the third attempt" do
          3.times{ SuperMemo2.new(@card, @DIFFICULTY).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(3)
        ...
        it "the fourth attempt" do
          4.times{ SuperMemo2.new(@card, @DIFFICULTY).check_translation(@inputed_text) }
          expect(@card[:current_step]).to eq(4)
    ...
    describe "misprint_case" do
      ...
      it "the first attempt" do
        SuperMemo2.new(@card, 0).check_translation(@inputed_text)
        expect(@card[:current_step]).to eq(1)
      ...
      it "the fifth attempt" do
        5.times{ SuperMemo2.new(@card, @BEFORE_HESITATION).check_translation(@inputed_text) }
        expect(@card[:current_step]).to eq(1)
        expect(@card[:e_factor]).to eq(1.3)
        expect(@card[:review_date]).to eq(1.day.from_now)
      end
